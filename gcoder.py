...
class Line(object):
    __slots__ = (
    'x', 'y', 'z', 'e', 'f', 'i', 'j', 's', 'p', 'raw', 'split_raw', 'command', 'is_move', 'relative', 'relative_e',
    'current_x', 'current_y', 'current_z', 'extruding', 'current_tool', 'gcview_end_vertex')
...
class GCode(object):
    ...
    def _preprocess_layers(self):
        ...
        for l in self.all_layers:
            (current_x, current_y, current_z), (xm, xM), (ym, yM), (zm, zM) = l._preprocess(current_x, current_y,
                                                                                            current_z)
            xmin = min(xm, xmin)
            xmax = max(xM, xmax)
            ymin = min(ym, ymin)
            ...
            for line in layer.lines:
                ...
                if line.command == "G4":
                    ...
                    if distance <= currenttravel and lastf + f != 0 and f != 0:
                        ...
                        moveduration = math.sqrt(
                            2 * distance / acceleration)  # probably buggy : not taking actual travel into account
